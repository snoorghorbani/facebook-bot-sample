...
app.post('/webhook/', function(req, res) {
	bot.sendTextMessage(req.body.entry[0].id, req.body.entry[0].messaging[0].message.text);
	res.sendStatus(200);
});
...
bot.setGreetingText(
	"Hello, My name is Soushians!, I'm here to help you manage your tasks. Be sure to setup your bucket by typing 'Setup'. "
);
...
bot.setGetStartedButton((payload, chat) => {
	if (config.bucket === undefined) {
		...
		chat.say(
			"It seems like you have not setup your bucket settings yet. That has to be done before you can do anything else. Make sure to type 'setup'"
		);
	}
...
bot.hear('help', (payload, chat) => {
	...
	chat.say("'quick_reply': for quick reply sample");
	chat.say("'buttons': for buttons sample");
	chat.say("'elements': for elements sample");
});

/**
 * quick reply sample
 */
bot.hear('quick_reply', (payload, chat) => {
	chat.say({
		text: 'Favorite color?',
...
bot.on('postback:FAVORITE_RED', (payload, chat) => {
 ...
 * quick reply sample
 */
bot.hear('elements', (payload, chat) => {
	chat.say({
		cards: [
			{ title: 'Artile 1', image_url: './1.jpg', default_action: {} }
			// { title: 'Artile 2', image_url: 'https://i.pinimg.com/564x/59/1a/2d/591a2d1ba968baaab1449524b157b41a.jpg', default_action: {} }
		],
		buttons: [ { type: 'postback', title: 'View More', payload: 'VIEW_MORE' } ]
	});
});
 ...
 * template sample
 */
bot.hear('template', (payload, chat) => {
	chat.sendTemplate({
